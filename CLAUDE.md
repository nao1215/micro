...
- [x] OpenAPI仕様書（docs/openapi.yaml）- Gateway公開API + 全内部サービスAPI
- [x] テストコード（全12パッケージ、pkg + internal全サービス）- `go test ./...` 全パス
- [x] フロントエンドUIとバックエンドの接続（dev-token認証、全タブのAPI接続、自動更新）

### 未着手
- [ ] Docker環境の動作確認（`docker compose up --build`）
- [ ] Lint通過確認（`make lint` - golangci-lintのインストールが必要）
- [ ] テストカバレッジの計測と改善
